...
public class Problem_1157_OnlineMajorityElementInSubarray {
    ...
    class MajorityChecker {
        ...
        public MajorityChecker(int[] arr) {
            ...
            qc = new QuickCounter(arr);
        }

        public int query(int left, int right, int threshold) {
            int candidate = st.query(left, right);
            return qc.realTimes(left, right, candidate) >= threshold ? candidate : -1;
        }
        ...
        class QuickCounter {
            ...
            public int realTimes(int left, int right, int num) {
                List<Integer> indies = cnt.get(num);
                return size(indies, right) - size(indies, left - 1);
            }

            private int size(List<Integer> indies, int index) {
                int l = 0;
                int r = indies.size() - 1;
                int m = 0;
                int ans = -1;
